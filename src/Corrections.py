...
def correct_nc_file(path, output_file_name_temp, file_start_date, file_end_date, last_processed_date):
    ...
    if file_start_date.month == Month.JANUARY:
        print(
            f'correct nc-file with \n cdo -selmon,1,2,3,4,5,6 {path}{input_file_name}.nc {path}{output_file_name}.nc')
        cdo.selmon(
            '1,2,3,4,5,6', input=f'{path}{input_file_name}.nc', output=f'{path}{output_file_name}.nc', options = '-f nc4c -z zip_5')

    elif file_start_date.month == Month.JULY:
        print(
            f'correct nc-file with \n cdo -selmon,6,7,8,9,10,11,12 {path}{input_file_name}.nc {path}{output_file_name}.nc')

        cdo.selmon('7,8,9,10,11,12',
                   input=f'{path}{input_file_name}.nc', output=f'{path}{output_file_name}.nc', options = '-f nc4c -z zip_5')
