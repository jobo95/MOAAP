...
def compute_history(object_container: ObjectContainer, threshold: float = 3) -> ObjectContainer:
    ...
    for type_ in ("start","end"):
        ...
        for i,obj in enumerate(all_objs):
            ...
            object_container = edit_obj_history(object_container,obj_index,largest_closest_obj_index, time, type_ = type_)
            
    return object_container

def calc_min_distance_between_objs(p1 : ndarray[list[RotatedGridPoint]],
                                   p2:ndarray[list[RotatedGridPoint]]) -> float:
...
def edit_obj_history(obj_container:ObjectContainer,obj_index:int, obj_close_index:int, time:np.datetime64, type_:str)->ObjectContainer:
